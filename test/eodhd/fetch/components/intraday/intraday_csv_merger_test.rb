...
describe Eodhd::IntradayCsvMerger do
  ...
  describe ".merge" do
    it "returns new rows when existing is nil" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(nil, new_rows, 100, 200)
      _(result).must_equal new_rows
    end

    it "returns new rows when existing is empty" do
      new_rows = [row(100), row(200)]
      result = Eodhd::IntradayCsvMerger.merge([], new_rows, 100, 200)
      _(result).must_equal new_rows
    end

    it "returns existing rows when new is nil" do
      existing_rows = [row(100), row(200)]
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, nil, 100, 200)
      _(result).must_equal existing_rows
    end

    it "returns existing rows when new is empty" do
      existing_rows = [row(100), row(200)]
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, [], 100, 200)
      _(result).must_equal existing_rows
    end
    ...
    it "handles no overlap - new data after existing" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 500, 600)
    ...
    it "handles no overlap - new data before existing" do
      ...
      new_rows = [row(100), row(200)]
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 100, 200)
    ...
    it "handles adjacent ranges - new data immediately after existing" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 301, 400)
    ...
    it "handles adjacent ranges - new data immediately before existing" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 100, 299)
    ...
    it "handles partial overlap - new data overlaps end of existing" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 250, 450)
      _(result).must_equal [row(100), row(250), row(350), row(450)]
    end
    ...
    it "handles partial overlap - new data overlaps start of existing" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 200, 450)
    ...
    it "handles new data completely within existing (subset)" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 250, 350)
    ...
    it "handles new data completely containing existing (superset)" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 100, 600)
    ...
    it "handles complete replacement - exact same range" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 100, 300)
    ...
    it "handles boundary inclusion - excludes start timestamp" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 200, 250)
    ...
    it "handles boundary inclusion - excludes end timestamp" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 250, 300)
      _(result).must_equal [row(100), row(200), row(250), row(300), row(400)]
    end
    ...
    it "handles replacing middle section with gap before and after" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 550, 750)
      _(result).must_equal [row(100), row(200), row(550), row(650), row(750), row(900), row(1000)]
    end

    it "handles exclude range covering all existing data" do
      existing_rows = [row(200), row(300), row(400)]
      new_rows = [row(100), row(500)]
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 100, 500)
    ...
    it "handles exclude range with no matching existing data" do
      ...
      result = Eodhd::IntradayCsvMerger.merge(existing_rows, new_rows, 400, 500)
